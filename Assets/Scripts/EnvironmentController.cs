...
public class EnvironmentController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Initial state: nothing selected yet, but manual panel should NOT show immediately
    ...
    public void StartScan()
    {
        ...
        if (scanTitleText != null) scanTitleText.text = scanningBaseText;
        if (infoText != null) infoText.text = "Move your phone around slowly";

        if (rescanButton != null) rescanButton.gameObject.SetActive(false);
    ...
    {
        if (mlClassifier != null && testTexture != null)
        {
            bool success = mlClassifier.TryPredict(testTexture, out predictedEnvironment, out confidence);
            if (success)
                return;
        }
    ...
    private void HandleScanResult(EnvironmentType predictedEnvironment, float confidence)
    {
        ...
        {
            StartCoroutine(HandleFirstFailure(predictedEnvironment, confidence));
        }
        else
        {
            StartCoroutine(HandleSecondFailure());
        }
    }
    ...
    private IEnumerator HandleFirstFailure(EnvironmentType predictedEnvironment, float confidence)
    {
        if (loadingIcon != null)
            loadingIcon.gameObject.SetActive(false);
        ...
        if (infoText != null)
            infoText.text = "Please move camera closer to surface";

        if (rescanButton != null)
            rescanButton.gameObject.SetActive(true);
    }

    private IEnumerator HandleSecondFailure()
    {
        if (loadingIcon != null)
            loadingIcon.gameObject.SetActive(false);
        ...
        if (errorModal != null)
        {
            ...
            if (errorModalText != null)
                errorModalText.text = "Unable to detect environment\nPlease choose the environment manually";
        }
    ...
    public void SetEnvironment(EnvironmentType environment)
    {
        ...
        // Keep manual selection hidden unless explicitly shown after second failure
        if (manualSelectionPanel != null)
            manualSelectionPanel.SetActive(false);
    }
